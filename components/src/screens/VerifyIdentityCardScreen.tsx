...
export default function VerifyIdentityCardScreen({
  ...
  return (
    <View style={s.wrap}>
      <LinearGradient colors={[COLORS.primaryLight, COLORS.primaryLight]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.card}>
        <View style={s.left}>
          <View style={s.iconCircle}>
            <Ionicons name={tone === "danger" ? "alert-circle-outline" : tone === "info" ? "time-outline" : "shield-checkmark-outline"} size={25} color={COLORS.primaryMid} />
          </View>
          <View style={s.textCol}>
            <View style={s.labelRow}>
              <View style={[s.dot, { backgroundColor: COLORS.primary }]} />
              <AppText style={[s.label, { color: COLORS.primary }]}>{tone === "info" ? "In review" : tone === "danger" ? "Action needed" : "Required"}</AppText>
            </View>
